...
function Carousel() {
  ...
  const styleBGL = {
    backgroundImage: `url(${
      image[idImage === image.length - 1 ? image.length - 1 : idImage + 1]
        .images
    })`,
  };
        ...
        {image.map((_, indx) => {
          return (
            <button
              key={indx}
              onClick={() =>
                indx <= image.length - 1 && indx >= 0
                  ? setIndikator(indx)
                  : setIndikator(0)
              }
              className={idImage === indx ? "indicator" : "indicator notactv"}
            >
              <span>s</span>
            </button>
          );
